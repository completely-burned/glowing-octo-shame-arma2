...
while {true} do {
	...
	if (alive _cursorTarget && side _cursorTarget == playerSide && _cursorTarget isKindOf "Man") then {
		...
		if (isNil "_join_action") then {
			_player = player;
			_join_action = _player addaction ["join group" + " " + name _cursorTarget, "m\functions\join_grp.sqf", _cursorTarget, 0.5, false, false, "","group cursorTarget != group _this && alive cursorTarget && side cursorTarget == playerSide && (effectiveCommander cursorTarget) isKindOf 'Man'"];
