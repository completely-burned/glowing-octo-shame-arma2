...
_r = [250, date select 0, 100];

// Тип.
if (_w in ["plr_bulge","mcn_neaville_winter","i44_merderet_winter","chernarus_winter",
	"gm_weferlingen_winter","tem_summawcup","thirskw"]) then {_r set [0,100]};
...
if (_w in ["fdf_isle1_a","chernarus","i44_omaha_v2","cup_chernarus_a3",
	"noe","thirsk","vt7","ww2_omaha_beach","spex_carentan","eden"]) then {_r set [0,190]};
...
if (_w in ["i44_omaha","i44_omaha_v2","mcn_neaville","i44_merderet_winter",
	...
	"spe_normandy","spe_mortain","spex_utah_beach","spex_lingevres"]) then
{
	_r set [1,1944];
};
if (_w in ["vn_khe_sanh","vn_the_bra","cam_lao_nam","dong_giang","vnx_rssz"]) then
{
	_r set [1,1968];
};
...
if (_w in ["zdanice","stozec","cain","teregova","eden"]) then
{
	_r set [1,1986];
};
if (_w in ["juju_javory"]) then
{
	_r set [1,1991];
};
if (_w in ["sara"]) then
