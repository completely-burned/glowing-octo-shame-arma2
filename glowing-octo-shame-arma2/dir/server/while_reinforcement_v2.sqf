		...
		if (gosa_pvp) then {
			...
			//--- enemyCoefficient
				if(!isNil {CivilianLocationStartTime})then{
					/*---------------------------------------------------------------------------
						[120, 4] параметры к примеру [время подкреплений , множитель]

						120 прошедшее время для локации
						120-120=0 осталось времени
						0/(120/4) =0 множитель с учетом времени

						100 прошедшее время для локации
						120-100 =20 осталось времени
						20/(120/4) =0.7 множитель с учетом времени

						30 прошедшее время для локации
						120-30=90 осталось времени
						90/(120/4) =3 множитель с учетом времени

						0.5 прошедшее время для локации
						120-0.5 =119.5 осталось времени
						119.5/(120/4) =3,98 множитель с учетом времени

						0 прошедшее время для локации
						120-0=120 осталось времени
						120/(120/4) =4 множитель с учетом времени


						[120, 0.5] параметры к примеру [время подкреплений , множитель]

						120 прошедшее время для локации
						120-120=0 осталось времени
						0/(120/0.5) =0 множитель с учетом времени

						100 прошедшее время для локации
						120-100 =20 осталось времени
						20/(120/0.5) =0,08 множитель с учетом времени

						30 прошедшее время для локации
						120-30=90 осталось времени
						90/(120/0.5) =0,375 множитель с учетом времени

						0.5 прошедшее время для локации
						120-0.5 =119.5 осталось времени
						119.5/(120/0.5) =0,49 множитель с учетом времени

						0 прошедшее время для локации
						120-0=120 осталось времени
						120/(120/0.5) =0,5 множитель с учетом времени
					---------------------------------------------------------------------------*/
