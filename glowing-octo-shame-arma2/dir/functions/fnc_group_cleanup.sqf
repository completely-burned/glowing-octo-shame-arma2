...
if ( isNil "_time" ) then {
	...
	_grp setVariable ["grp_created_time", _time];
		diag_log format ["gosa_fnc_group_cleanup.sqf %1 grp_created_time %2", _grp, _time ];
};
	...
	_assignedVehicles = [];
	{
		...
		diag_log format ["gosa_fnc_group_cleanup.sqf units %1", _units];
	...
	if({alive _x} count _units > 0)then{
		...
		if( ({!isNil {_x getVariable "gosa_transportwaypoint_created_GET_IN_pos"}} count [_grp] + _vehicles > 0 ) or ({!isNil {_x getVariable "gosa_transportwaypoint_created_GET_OUT_pos"}} count [_grp] + _vehicles > 0 ))then{
			breakTo "main";
				diag_log format ["gosa_fnc_group_cleanup.sqf %1 breakTo main, transport", _grp ];
		};
		...
		if (true) then {
			...
			if(_leader call gosa_fnc_isPlayer)then{
				...
				if(!isNil {_grp getVariable "grp_created"})then{
						diag_log format ["gosa_fnc_group_cleanup.sqf %1  группа готова", _grp ];
					...
					if(isNil "_cleanup")then{
						...
						_grp setVariable ["_cleanup",_cleanup];
							diag_log format ["gosa_fnc_group_cleanup.sqf %1 _cleanup init %2", _grp, _cleanup ];
					};
					...
					if(!isNil "_cleanup")then{
						...
						if([waypointPosition [_grp,_currentWP], _oldPosWP] call BIS_fnc_distance2D < 5 )then{ // если маршрут не менялся
								diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся", _grp ];
							if(waypointType [_grp, _currentWP] in ["SUPPORT"])then{ // поддержка
								...
								_grp setVariable ["_cleanup",_cleanup];
									diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, поддержка, местоположение перезаписано, таймер +30 +120", _grp ];
							};
							if(currentCommand _leader == "FIRE AT POSITION")then{ // артиллерия
								...
								_cleanup = [getPos vehicle _leader,_oldTime,_oldTime2,waypointPosition [_grp,_currentWP]];
								_grp setVariable ["_cleanup",_cleanup];
									diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, артиллерия, местоположение перезаписано, таймер +30 +120", _grp ];
							};
							if({currentCommand _x in ["ATTACK","FIRE","ATTACKFIRE"]} count _units > 0 )then{ // ATTACK
								...
								_cleanup = [getPos vehicle _leader,_oldTime,_oldTime2,waypointPosition [_grp,_currentWP]];
								_grp setVariable ["_cleanup",_cleanup];
									diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, атакует, местоположение перезаписано, таймер +30 +60", _grp ];
							};
							if({currentCommand _x in ["GET OUT","GET IN"]} count _cargo+_units > 0 )then{ // GET OUT
								...
								_cleanup = [getPos vehicle _leader,_oldTime,_oldTime2,waypointPosition [_grp,_currentWP]];
								_grp setVariable ["_cleanup",_cleanup];
									diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, входвыход с транспортом, местоположение перезаписано, таймер +20 +40", _grp ];
							};

							if(_oldTime < time)then{ // 1 таймер
									diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, 1 таймер сработал", _grp ];
								if(_oldPos distance _leaderPos >= 5)then{ // сдвинулись
										diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, группа сдвинулась когда 1 таймер работал", _grp ];
									if((vehicle _leader distance civilianBasePos) <= (sizeLocation / 2 + sizeLocation))then{ // на точке
										...
										_grp setVariable ["_cleanup",_cleanup];
											diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, группа на точке, сдвинулись, местоположение перезаписано, таймер +120 +240", _grp ];
									}else{
										_oldTime = _oldTime max time+30;
										_oldTime2 = _oldTime2 max time+120;
										_cleanup = [getPos vehicle _leader,_oldTime,_oldTime2,waypointPosition [_grp,_currentWP]];// 1 и 2 таймер обновление
										_grp setVariable ["_cleanup",_cleanup];
											diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, группа не на точке, сдвинулись, местоположение перезаписано, таймер +30 +120", _grp ];
									};
								}else{
										diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, группа не сдвинулась когда 1 таймер работал, удаляю маршруты, пересоздадутся", _grp ];
									...
									if(_Helicopter)then{
										...
										_grp setVariable ["_cleanup",_cleanup];
											diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, _Helicopter, группа не сдвинулась когда 1 таймер работал, местоположение перезаписано, таймер +10 --", _grp ];
									}else{
										if((vehicle _leader distance civilianBasePos) <= (sizeLocation / 2 + sizeLocation))then{ // на точке
											_oldTime = _oldTime max time+120;
											_oldTime2 = _oldTime2 max time+240;
											_cleanup = [getPos vehicle _leader,_oldTime,_oldTime2,waypointPosition [_grp,_currentWP]]; // 1 таймер обновление
											_grp setVariable ["_cleanup",_cleanup];
												diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, группа на точке, группа не сдвинулась когда 1 таймер работал, местоположение перезаписано, таймер +120 +240", _grp ];
										}else{
											_oldTime = _oldTime max time+30;
											_cleanup = [getPos vehicle _leader,_oldTime,_oldTime2,waypointPosition [_grp,_currentWP]]; // 1 таймер обновление
											_grp setVariable ["_cleanup",_cleanup];
												diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, группа не на точке, группа не сдвинулась когда 1 таймер работал, местоположение перезаписано, таймер +30 --", _grp ];
										};
									};
								};
							};
							...
							if(_oldTime2 < time)then{ // 2 таймер
								{_x setVariable ["gosa_timeDeleteVehicle", 0]}forEach units _grp; // на удаление
									diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут не менялся, 2 таймер сработал, в очередь на удаление", _grp ];
							};
							...
							_grp setVariable ["_cleanup",_cleanup];
								diag_log format ["gosa_fnc_group_cleanup.sqf %1  маршрут изменился, местоположение перезаписано", _grp ];
						};
					};

				}else{
						diag_log format ["gosa_fnc_group_cleanup.sqf %1  группа не готова", _grp ];
					if ( _time + 30 < time )then {
						_grp setVariable ["grp_created",true];
							diag_log format ["gosa_fnc_group_cleanup.sqf %1  группа 30 сек grp_created true", _grp ];
					};
				};
	...
	diag_log format ["gosa_fnc_group_cleanup end %1", time];
