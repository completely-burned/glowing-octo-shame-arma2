...
for "_i" from 0 to ((count (configFile >> "CfgVehicles")) - 1) do {
	...
	if (isClass _entry) then {
		if ((getNumber(_entry >> "scope")) == 2) then {
			if (!(((getNumber(_entry >> "side")) call gosa_fnc_getSide) in [west,east,resistance]) or (_friendly_vehicles_only == 0) or !isMultiplayer or ((getNumber(_entry >> "side")) call gosa_fnc_getSide) in m_friendlySide) then {
