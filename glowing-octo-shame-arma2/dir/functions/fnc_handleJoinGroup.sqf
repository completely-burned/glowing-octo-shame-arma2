...
while {true} do {
	...
	if (alive _cursorTarget && side _cursorTarget == playerSide && _cursorTarget isKindOf "Man") then {
		...
		if (isNil "_join_action") then {
			_player = player;
			_join_action = _player addaction [localize "STR_gosa_join_the_squad" + " " + name _cursorTarget, "dir\actions\act_join_grp.sqf", _cursorTarget, 0.5, false, false, "","group cursorTarget != group _this && alive cursorTarget && side cursorTarget == playerSide && (effectiveCommander cursorTarget) isKindOf 'Man'"];
